				    ...
				    enum led_brightness value)
{
	...
	if (brightness > 200) {
	//torch mode lct.huk debug
		flash_en =
			flash_led->ctrl_seq[FLASH_EN].flash_on_val;
        while(j < 8)
        {
                gpio_direction_output(flash_led->flash_now, 2);
		udelay(1);
                j++;
	...
	if (rc) {
        ...
        printk("<3>""%s:brightness %d, flash_en=%d, flash_now=%d\n", __func__,
                        brightness, flash_en, flash_now);
...
static int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	pr_err("#################HDJ flash_led->flash_on = %d \n",flash_led->flash_en);
...
MODULE_ALIAS("leds:leds-msm-back-gpio-flash");
