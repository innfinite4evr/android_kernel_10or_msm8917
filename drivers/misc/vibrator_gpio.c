...
static int __init vibrator_gpio_init(void)
{
	pr_err("vibrator gpio init\n");
	return platform_driver_register(&vibrator_gpio_driver);
}

static void __init vibrator_gpio_exit(void)
{
	platform_driver_unregister(&vibrator_gpio_driver);
}

//module_platform_driver(vibrator_gpio_driver);

module_init(vibrator_gpio_init);
module_exit(vibrator_gpio_exit);
