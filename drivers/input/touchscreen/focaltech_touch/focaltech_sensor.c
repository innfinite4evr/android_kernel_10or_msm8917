...
int fts_poll_delay(struct sensors_classdev* proximity_cdev, unsigned int delay_ms);//copyed fix prox hal layer can not receive event by Simon 20161226
/*****************************************************************************
* Static variables
*****************************************************************************/
static struct sensors_classdev __maybe_unused sensors_proximity_cdev =
{
    ...
    .flags =3, //copyed fix prox hal layer can not receive event by Simon 20161226
    .enabled = 0,
    .delay_msec = 200,
    .sensors_enable = NULL,
...
/*************************************************************
*  Name: fts_psensor_enable_store
*  Brief:
*  Input:
*  Output:
*  Return:
**************************************************************/
int fts_psensor_enable_store(struct fts_ts_data* data,
                                  unsigned int enable)
{
   struct input_dev *input_dev = data->psensor_pdata->input_psensor_dev;
    mutex_lock(&input_dev->mutex);
    //pr_err("lxf [%s == %d] enable =%d \n", __func__, __LINE__, enable);
    fts_psensor_enable(data, enable);
    //data->psensor_pdata->tp_psensor_data = FTS_PSENSOR_ORIGINAL_STATE_FAR;
    if (enable)
        data->psensor_pdata->tp_psensor_opened = 1;
    else
        data->psensor_pdata->tp_psensor_opened = 0;
                                  ...
                                  unsigned int enable)
{
    ...
    if (enable)
    {
        ...
        if (data->psensor_pdata->input_psensor_dev != NULL)
        {
...
/*****************************************************************************
*  Name: fts_poll_delay
*  Brief: add for sensor poll_delay
*  Input:
*  Output:
*  Return:  //fix prox hal layer can not receive event by Simon 20161226
*****************************************************************************/

int fts_poll_delay(struct sensors_classdev* proximity_cdev, unsigned int delay_ms)//copyed fix prox hal layer can not receive event by Simon 20161226
{
   //DBG(&g_I2cClient->dev, "*** %s() delay_ms = %d ***\n", __func__, delay_ms);
   	FTS_ERROR("%s lxf delay_ms =%d\n",__func__,delay_ms);
   	mdelay(delay_ms);
...
static int fts_read_tp_psensor_data(struct fts_ts_data *data)
{
    ...
    {
        FTS_ERROR("%s sensor data changed [0x%X]", __func__, psensor_status);
        ret = 0;
    }
    return ret;
}
...
int fts_sensor_init(struct fts_ts_data *data)
{
    ...
    if (fts_psensor_support_enabled() )
    {
        ...
        //psensor_input_dev = input_allocate_device();
         psensor_input_dev  = devm_input_allocate_device(&data->client->dev);
         //copyed fix prox hal layer can not receive event by Simon 20161226
        ...
        psensor_input_dev->phys = "I2C";
        psensor_input_dev->id.bustype = BUS_I2C;
        //psensor_input_dev->dev.parent = &data->client->dev; //copyed fix prox hal layer can not receive event by Simon 20161226
        ...
        psensor_pdata->ps_cdev.sensors_poll_delay = fts_poll_delay;
        psensor_pdata->data = data;

        err = sensors_classdev_register(&psensor_input_dev->dev, &psensor_pdata->ps_cdev); //copyed fix prox hal layer can not receive event by Simon 20161226
        if (err)
        {
