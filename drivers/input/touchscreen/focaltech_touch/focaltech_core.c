...
struct mutex ft6336u_resume_mutex;
...
//add lujian for Psensor can not work when calling
extern int fts_psensor_enable_store(struct fts_ts_data* data,
                                  unsigned int enable);
static bool psensor_call_resume_flag;
static bool psensor_call_suspend_flag;
//add end
...
int fts_wait_tp_to_valid(struct i2c_client *client)
{
    ...
    /* If only the wrong app, program in Room Boot. No need to quit */
    ret = fts_ctpm_get_pram_or_rom_id(client);
    if(FTS_RUN_IN_ERROR != ret) {
        FTS_ERROR("fts_ctpm_get_pram_or_rom_id: %d", ret);
	return 0;
    }
...
#if FTS_REPORT_PRESSURE_EN
            FTS_POINT("[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!", event->au8_finger_id[i], event->au16_x[i],
                      event->au16_y[i], event->pressure[i], event->area[i]);
#else
            FTS_POINT("[B]P%d(%d, %d)[tm:%d] DOWN!", event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], event->area[i]);
#endif
        }
        else
        {
...
#endif
            data->touchs &= ~BIT(event->au8_finger_id[i]);
            FTS_POINT("[B]P%d UP!", event->au8_finger_id[i]);
        }
    }
...
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    FTS_POINT("point number: %d, touch point: %d", event->point_num,
              event->touch_point);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 ...
 fts_ctpm_get_upgrade_array();
    ...
    fts_reset_proc(200);
     ...
     if (err < 0)
     {
         FTS_ERROR("Can not Get correct Device ID");
         goto free_gpio;
     }
    ...
    err = fts_input_dev_init(client, data, input_dev, pdata);
    if (err)
    {
        FTS_ERROR("Input device registration failed");
        goto free_input_dev;
...
sprintf(fw_version, "[FW]0x%02x,[IC]FT6336U", data->fw_ver);
init_tp_fm_info(0, fw_version, "focaltech");
// added end

    INIT_WORK(&g_resume_work, do_ts_resume_work);
    mutex_init(&ft6336u_resume_mutex);
...
#endif
// add by lujian to solve tp can not work during open psensor fuc

	 psensor_call_resume_flag = false;
	 psensor_call_suspend_flag= false;

    if (data->psensor_pdata->tp_psensor_opened) //if tp_psensor_opened == 1  during call
    {
        psensor_call_resume_flag = false;
        psensor_call_suspend_flag= true;
       // pr_err("lxf [%s]psensor_call_resume_flag ==%d \n", __func__, psensor_call_resume_flag);
        //pr_err("lxf [%s]psensor_call_suspend_flag ==%d \n", __func__, psensor_call_suspend_flag);
        data->suspended = true;
        return 0;
}
// add end
...
{
    // pr_err("lxf [%s == %d] bf fts_psensor_enable_store \n", __func__, __LINE__);
     fts_psensor_enable_store(data, true);
    // pr_err("lxf [%s == %d] af fts_psensor_enable_store \n", __func__, __LINE__);
}
#if 0
...
#endif
// add by lujian to solve tp can not work during open psensor fuc
if (psensor_call_suspend_flag==1) // during call
{

    psensor_call_suspend_flag =0; //reset flag
    psensor_call_resume_flag  =1;
    data->suspended = false;
    //pr_err("lxf [%s]psensor_call_resume_flag ==%d \n", __func__, psensor_call_resume_flag);
    //pr_err("lxf [%s]psensor_call_suspend_flag ==%d \n", __func__, psensor_call_suspend_flag);
      return 0;
