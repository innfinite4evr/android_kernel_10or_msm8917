...
#ifdef SUPPORT_READ_TP_VERSION
char product_id[10];//[chip type]
u16 g_tp_cfg_ver = -1;//[FW]
u16 g_sensor_id = -1;//[vendor]
#endif
...
ssize_t DrvMainProcfsCustomerFirmwareVersionWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &_gFwVersion);//nMajor ==>[vendor];nMinor ==> [FW]
...
/*
    for (i = 0; i < _gDebugReadDataSize; i ++) // Output format 1.
    {
	...
	if(bCallApkToGetFwLog==1)
	{
        ...
        else //g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE || g_FirmwareMode == FIRMWARE_MODE_RAW_DATA_MODE
        {
            if (g_LogModePacket != NULL)
            {
                ...
                if ((g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) && (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE) && (g_LogModePacket[0] == 0xA5 || g_LogModePacket[0] == 0xAB || g_LogModePacket[0] == 0xA7))
                {
...
//------------------------------------------------------------------------------//
#ifdef SUPPORT_READ_TP_VERSION
void getTpFwVersion(void)
{
    char fw_version[64];

    DrvIcFwLyrGetCustomerFirmwareVersion(&g_sensor_id, &g_tp_cfg_ver, &_gFwVersion);//nMajor ==>[vendor];nMinor ==> [FW]

    DBG(&g_I2cClient->dev, "*** %s() _gFwVersion = %s ***\n", __func__, _gFwVersion);
    DBG(&g_I2cClient->dev, "*** %s() g_sensor_id = %d ***\n", __func__, g_sensor_id);
    DBG(&g_I2cClient->dev, "*** %s() g_tp_cfg_ver = %d ***\n", __func__, g_tp_cfg_ver);

	memset(fw_version, 0, sizeof(fw_version));
	sprintf(fw_version, "[FW]0x%02x,[IC]MSG0x%02x", g_tp_cfg_ver, g_ChipType);
	switch(g_sensor_id){
		case 0x0010: init_tp_fm_info(0, fw_version, "HOLITECH");break;
		case 0x0002: init_tp_fm_info(0, fw_version, "Djin");break;
		}
}
#endif
...
s32 DrvMainTouchDeviceInitialize(void)
{
    ...
    {
//////terry start	fix the reboot issue when remove touch panel start
...
#endif //CONFIG_ENABLE_ITO_MP_TEST fix the reboot issue when remove touch panel end
