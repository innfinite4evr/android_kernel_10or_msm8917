...
static struct sensors_classdev sensors_proximity_cdev = {
    ...
    .flags = 3,	//fix prox hal layer can not receive event by Simon 20161226
    .enabled = 0,
    .delay_msec = 200,
    .sensors_enable = NULL,
...
/*=============================================================*/
int tp_has_powered = 0;/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/

#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
extern u8 screen_is_off;
#endif 
#ifdef CONFIG_TP_HAVE_KEY
int g_TpVirtualKey[] = {TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_MENU, TOUCH_KEY_SEARCH};
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
int Poll_Delay(struct sensors_classdev* pProximityCdev, unsigned int delay_ms);//fix prox hal layer can not receive event by Simon 20161226
...
static struct device_attribute proximity_attribute = __ATTR(proximity, 0664, _DrvPlatformLyrProximityDetectionShow, _DrvPlatformLyrProximityDetectionStore);
...
static int _DrvPlatformLyrProximityInputDeviceInit(struct i2c_client *pClient)
{
    ...
    //fix prox hal layer can not receive event by Simon 20161226 start
    //g_ProximityInputDevice = input_allocate_device();
    g_ProximityInputDevice = devm_input_allocate_device(&pClient->dev);
    //fix prox hal layer can not receive event by Simon 20161226 end
    ...
    //g_ProximityInputDevice->dev.parent = &pClient->dev;     //fix prox hal layer can not receive event by Simon 20161226 
    g_ProximityInputDevice->id.bustype = BUS_I2C;
    ...
    //fix prox hal layer can not receive event by Simon 20161226 start
    sensors_proximity_cdev.sensors_poll_delay= Poll_Delay;
    //nRetVal = sensors_classdev_register(&pClient->dev, &sensors_proximity_cdev);
    nRetVal = sensors_classdev_register(&g_ProximityInputDevice->dev, &sensors_proximity_cdev);
    //fix prox hal layer can not receive event by Simon 20161226 end
...
/*  fix the reboot issue when remove touch panel
static int _DrvPlatformLyrProximityInputDeviceUnInit(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
...
static ssize_t _DrvPlatformLyrVirtualKeysShow(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    return sprintf(buf,
        __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":50:1330:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE) ":150:1330:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(580/*KEY_MENU*/) ":250:1330:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":350:1330:100:100"
        "\n");
   #else
    return sprintf(buf,
        __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":50:850:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE) ":150:850:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(580/*KEY_MENU*/) ":250:850:100:100"
        ":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":350:850:100:100"
        "\n");
   #endif
}
...
//    printk("*** kls DrvIcFwLyrHandleFingerTouch interrupt entry***\n");  // add for debug
    DrvIcFwLyrHandleFingerTouch(NULL, 0);
//    printk("*** kls DrvIcFwLyrHandleFingerTouch interrupt done***\n");  // add for debug
...
static irqreturn_t _DrvPlatformLyrFingerTouchInterruptHandler(s32 nIrq, void *pDeviceId)
{
    ...
    //DBG(&g_I2cClient->dev, "***[Simon IRQ Handle] %s() ***\n", __func__); 
...
/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
    tp_has_powered = 1;
    DBG(&g_I2cClient->dev, "*** %s() [Simon]tp_has_powered = %d ***\n", __func__, tp_has_powered);
/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
}
...
/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
//[L2000]start by xiongshaohua to disable proximity detection 
    #ifndef CONFIG_S502_BOARD_SUPPORT
    if(g_EnableTpProximity == 1){
        printk("***kls %s() g_EnableTpProximity = %d,return!!!***\n", __func__, g_EnableTpProximity);
        return;
    }
	#endif
//[L2000]end by xiongshaohua to disable proximity detection 
    tp_has_powered = 0;
    DBG(&g_I2cClient->dev, "*** %s() [Simon]tp_has_powered = %d ***\n", __func__, tp_has_powered);
/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
...
{
  //  printk("kls *** %s() ***\n", __func__); 
  //  printk("kls point touch pressed\n"); 
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
int Poll_Delay(struct sensors_classdev* pProximityCdev, unsigned int delay_ms)
{
	DBG(&g_I2cClient->dev, "*** %s() delay_ms = %d ***\n", __func__, delay_ms); 
	mdelay(delay_ms);
...
int DrvPlatformLyrTpPsEnable(struct sensors_classdev* pProximityCdev, unsigned int nEnable)
{
    ...
    {    
/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
	DBG(&g_I2cClient->dev, "*** Enter %s() [Simon]tp_has_powered = %d ***\n", __func__, tp_has_powered);
	
	if(tp_has_powered  == 0){
	DrvPlatformLyrTouchDeviceRegulatorPowerOn(1);
