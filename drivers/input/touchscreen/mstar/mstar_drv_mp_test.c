...
static u16 _DrvMpTestItoTestMsg21xxaGetDataOut(s16 *pRawData)
{
    ...
    mdelay(100);
    pr_err("lxf [%s==%d] SLAVE_I2C_ID_DBBUS =%d \n", __func__, __LINE__,SLAVE_I2C_ID_DBBUS);
    for (i = 0; i < nSensorNum * 2; i ++)
    {
        DBG(&g_I2cClient->dev, "[lxf]szDbBusRxData[%d] = %d\n", i, szDbBusRxData[i]); // add for debug
    }
...
static u16 _DrvMpTestItoTestMsg22xxGetDataOut(s16 *pRawData, u16 nSubFrameNum)
{
    ...
    do 
    {
	  ...
	  DBG(&g_I2cClient->dev,"*** nRegInt=  %x ***\n",nRegInt);

	  DBG(&g_I2cClient->dev,"*** try count i=  %d ***\n",i);
...
static void _DrvMpTestItoOpenTestMsg22xxFirst(u8 nItemId, s16 *pRawData, s8 *pDataFlag)		
{
	...
	DBG(&g_I2cClient->dev, "error_flag = %d\n",  error_flag);
    	if(error_flag==0)
   	{		
		return;
    	} 
...
static u16 _DrvMpTestItoTestSelfICChooseTpType()
{
    u16 nTpType = 0;
    u32 i = 0;
...
static ItoTestResult_e _DrvMpTestSelfICItoOpenTestEntry(void) 
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG22XX)
    {
		...
		if(error_flag==0)
		{	
		nRetVal1  =ITO_TEST_FAIL;
		nRetVal2 = ITO_TEST_FAIL;
		nRetVal3 = ITO_TEST_FAIL;
		goto ITO_TEST_END;
		}	
...
static ItoTestResult_e _DrvMpTestSelfICItoShortTestEntry(void)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG22XX)
    {
		...
		nRetVal2 = ITO_TEST_FAIL;
		nRetVal3 = ITO_TEST_FAIL;
		nRetVal4 = ITO_TEST_FAIL;
		nRetVal5 = ITO_TEST_FAIL;
		goto ITO_TEST_END;
		}		
...
static s32 _DrvMpTestItoTestMsg26xxmSwitchFwMode(u16 nFWMode)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
    ...
    switch (nFWMode)
    {
        ...
        case MSG26XXM_WATERPROOF:
            RegSet16BitValue(0x3CE4, 0x7992);
            DBG(&g_I2cClient->dev, "*** Msg26xx MP Test# MSG26XXM_WATERPROOF mode***\n");
            break;
...
static s32 _DrvMpTestItoTestMsg26xxmSwitchMode(u16 nSwitchMode, u16 nFMode)
{
    if (_gMutualICTestSwitchMode != 0)
    {
        if (_DrvMpTestItoTestMsg26xxmSwitchFwMode(nFMode) < 0)
        {
            if (nFMode == MSG26XXM_MUTUAL)
            {
...
static s32 _DrvMpTestMsg26xxmItoOpenTestEntry(void)
{
    ...
    _DrvMpTestItoTestMsg26xxmSwitchMode(_gMutualICTestSwitchMode, MSG26XXM_MUTUAL);
...
s32 _DrvMpTestMsg26xxmItoWaterProofTestEntry(void)
{
    ...
    if (_DrvMpTestItoTestMsg26xxmSwitchMode(_gMutualICTestSwitchMode, MSG26XXM_WATERPROOF) < 0)
    {
...
static u16 _DrvMpTestMsg28xxGetFirmwareOfficialVersionOnFlash(void)
{
	...
	DrvFwCtrlMsg28xxReadEFlashStart(0x81FE, EMEM_INFO);
	nReadAddr = 0x81FE;
    ...
    else
    {
        ...
        else
        {
            if (((g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX) && (nRetVal == ITO_TEST_FAIL)) 
            	|| ((g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX/* || g_ChipType == CHIP_TYPE_MSG58XXA*/) && (nRetVal == -1)))
            {
            	...
            	|| ((g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX/* || g_ChipType == CHIP_TYPE_MSG58XXA*/) && (nRetVal == -2)))
            {
