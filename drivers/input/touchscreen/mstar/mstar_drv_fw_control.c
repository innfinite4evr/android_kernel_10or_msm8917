...
#ifdef CONFIG_S502_BOARD_SUPPORT
#include "msg28xx_xxxx_update_l2000_bin.h"
#include "msg28xx_yyyy_update_l2000_bin.h"
#else
...
u8 g_DebugModePacket[MUTUAL_DEBUG_MODE_PACKET_LENGTH] = {0}; // for MSG21xxA/MSG22xx : DEBUG_MODE_PACKET_LENGTH = SELF_DEBUG_MODE_PACKET_LENGTH, for MSG26xxM/MSG28xx : DEBUG_MODE_PACKET_LENGTH = MUTUAL_DEBUG_MODE_PACKET_LENGTH
...
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareC33(u8 szFwData[][1024], EmemType_e eEmemType);
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareC32(u8 szFwData[][1024], EmemType_e eEmemType);
#endif //CONFIG_UPDATE_FIRMWARE_BY_SW_ID
...
/*=============================================================*/
static void _DrvFwCtrlMsg21xxaEraseEmemC33(EmemType_e eEmemType)
{
...
int apk_printf(unsigned char *pPacket)
{
	 ...
	 if (pPacket[0] == 0x2C)
	{
		if(pPacket[1]==1)
		{
			for(i=0;i<(SELF_DEMO_MODE_PACKET_LENGTH-2);i++)
			{
				fwLog[i+cnt*(SELF_DEMO_MODE_PACKET_LENGTH-2)] = pPacket[2+i];
			}
			cnt=0;
...
//        if (nButton)
        if (nButton != 0xFF)
        {
...
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
            printk("kls g_EnableTpProximity = %d, pPacket[nLength-2] = 0x%x\n", g_EnableTpProximity, pPacket[nLength-2]);
            ...
            if (g_EnableTpProximity && ((pPacket[nLength-2] == 0x80) || (pPacket[nLength-2] == 0x40)))
            {
                ...
                if ((nErr = hwmsen_get_interrupt_data(ID_PROXIMITY, &tSensorData)))
                {
                    printk("kls call hwmsen_get_interrupt_data() failed = %d\n", nErr);
                }
            ...
            for (i = 0; i < MAX_KEY_NUM; i ++)
            {
                if ((nButton & (1<<i)) == (1<<i))
                {
		         ...
		         printk("kls keyX=%d;keyY=%d\n",pInfo->tPoint[0].nX,pInfo->tPoint[0].nY);
	#endif //CONFIG_ENABLE_REPORT_KEY_WITH_COORDINATE
...
static u16 _DrvFwCtrlMsg21xxaGetSwId(EmemType_e eEmemType) 
{
    ...
    IicWriteData(SLAVE_I2C_ID_DWI2C, &szTxData[0], 5);
    IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 4);

    DBG(&g_I2cClient->dev, "szRxData[0,1,2,3] = 0x%x,0x%x,0x%x,0x%x\n", szRxData[0], szRxData[1], szRxData[2], szRxData[3]);

    if ((szRxData[0] >= 0x30 && szRxData[0] <= 0x39)
        &&(szRxData[1] >= 0x30 && szRxData[1] <= 0x39)
        &&(szRxData[2] >= 0x31 && szRxData[2] <= 0x39))  
    {
        nRetVal = (szRxData[0]-0x30)*100+(szRxData[1]-0x30)*10+(szRxData[2]-0x30);
    }
...
//    szTxData[1] = 0x00;
//    szTxData[2] = 0x00;    
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 1);
}	

static void _DrvFwCtrlMsg28xxIspBurstWriteEFlashDoWrite(u8 *pBufferData, u32 nLength)
{
    u32 i;
    u8  szTxData[3+MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE] = {0};
    ...
    for (i = 0; i < nLength; i ++)
    {
        szTxData[3+i] = pBufferData[i];
    }
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+nLength);
}	
...
void DrvFwCtrlMsg28xxReadEFlashStart(u16 nStartAddr, EmemType_e eEmemType)
{
    DBG(&g_I2cClient->dev, "*** %s() eEmemType = %d ***\n", __func__, eEmemType);
...
void DrvFwCtrlMsg28xxReadEFlashDoRead(u16 nReadAddr, u8 *pReadData)
{
...
void DrvFwCtrlMsg28xxReadEFlashEnd(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
static u16 _DrvFwCtrlMsg28xxGetSwId(EmemType_e eEmemType) 
{
    ...
    if (eEmemType == EMEM_MAIN) // Read SW ID from main block
    {
        DrvFwCtrlMsg28xxReadEFlashStart(0x7FFD, EMEM_MAIN);
        nReadAddr = 0x7FFD;
    }
    else if (eEmemType == EMEM_INFO) // Read SW ID from info block
    {
        DrvFwCtrlMsg28xxReadEFlashStart(0x81FB, EMEM_INFO);
        nReadAddr = 0x81FB;
    }

    DrvFwCtrlMsg28xxReadEFlashDoRead(nReadAddr, &szTmpData[0]);

    DrvFwCtrlMsg28xxReadEFlashEnd();
...
static s32 _DrvFwCtrlUpdateFirmwareCash(u8 szFwData[][1024], EmemType_e eEmemType)
{
...
#endif //CONFIG_ENABLE_CHIP_TYPE_MSG21XXA            
#else
            return _DrvFwCtrlMsg21xxaUpdateFirmwareC33(szFwData, EMEM_MAIN);
#endif //CONFIG_UPDATE_FIRMWARE_BY_SW_ID   
        }
        else
        {
...
#endif //CONFIG_ENABLE_CHIP_TYPE_MSG21XXA            
#else
            return _DrvFwCtrlMsg21xxaUpdateFirmwareC32(szFwData, EMEM_ALL);
#endif //CONFIG_UPDATE_FIRMWARE_BY_SW_ID        
        }
    }
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) // (0x85) || (0xBF)
    {
...
static s32 _DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    ...
    if ((eSwId == eVendorId) || (IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED))
    {
        ...
        else if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) && fsize == 133120/* 130KB */)
        {
...
void DrvFwCtrlWriteDQMemValue(u16 nAddr, u32 nData)
{
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)
    {
        ...
        szTxData[0] = 0x10;
        szTxData[1] = ((nAddr >> 8) & 0xff);
        szTxData[2] = (nAddr & 0xff);
        szTxData[3] = nData & 0x000000FF;
        szTxData[4] = ((nData & 0x0000FF00) >> 8);
        szTxData[5] = ((nData & 0x00FF0000) >> 16);
        szTxData[6] = ((nData & 0xFF000000) >> 24);
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 7);
    ...
    while (nRemainSize > 0)
    {
        ...
        if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05)
        {
            ...
            for (j = 0; j < nBlockSize; j ++)
            {
                for (k = 0; k < 3; k ++)
                {
                    szTxData[3+(j*3)+k] = _gOneDimenFwData[index+(i*nSizePerWrite)+j];
                }
                nSize = nSize + 3;
            }
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
            ...
            for (j = 0; j < nBlockSize; j ++)
            {
                for (k = 0; k < 4; k ++)
                {
                    szTxData[3+(j*4)+k] = _gOneDimenFwData[index+(i*nSizePerWrite)+j];
                }
                nSize = nSize + 4;
            }
            ...
            for (j = 0; j < nBlockSize; j ++)
            {
                szTxData[3+j] = _gOneDimenFwData[index+(i*nSizePerWrite)+j];
                nSize ++; 
            }
#endif //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A
        }
        ...
        rc = IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], nSize);
        if (rc < 0)
        {
...
static u32 _DrvFwCtrlMsg21xxaRetrieveMainCrcFromMainBlock(void) 
{
    ...
    nRetVal = szRxData[0];
    nRetVal = (nRetVal << 8) | szRxData[1];
    nRetVal = (nRetVal << 8) | szRxData[2];
    nRetVal = (nRetVal << 8) | szRxData[3];
...
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareBySwId(u8 szFwData[][1024], EmemType_e eEmemType) 
{
    ...
    // erase main
    _DrvFwCtrlMsg21xxaEraseEmemC33(EMEM_MAIN);
    mdelay(1000);
...
static u32 _DrvFwCtrlMsg26xxmCalculateInfoCrcByDeviceDriver(void)
{
    ...
    szTxData[0] = 0x72;
    szTxData[3] = 0x00; // read 128 bytes
    szTxData[4] = 0x80;
    ...
    for (i = 0; i < 8; i ++)
    {
        for (j = 0; j < 8; j ++)
        {
            szTxData[1] = 0x80 + (i*0x04) + (((j*128)&0xff00)>>8);
            szTxData[2] = (j*128)&0x00ff;
...
{
	extern void getTpFwVersion(void);
 	getTpFwVersion();
}
#endif
        DrvPlatformLyrEnableFingerTouchReport();

        if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)    
        {
...
#ifndef CONFIG_UPDATE_FIRMWARE_BY_SW_ID
static void _DrvFwCtrlMsg21xxaEraseEmemC32(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
static void _DrvFwCtrlReadInfoC33(void)
{
    ...
    szTxData[2] = 0xE6;
    szTxData[3] = 0x00;
    IicWriteData(SLAVE_I2C_ID_DBBUS, szTxData, 4);    
    mdelay(100);
    ...
    szTxData[0] = 0x72;
    szTxData[3] = 0x00;
    szTxData[4] = 0x80; // read 128 bytes

    for (i = 0; i < 8; i ++)
    {
        szTxData[1] = 0x80 + (((i*128)&0xff00)>>8);
        szTxData[2] = (i*128)&0x00ff;
    ...
    szTxData[1] = 0x80;
    szTxData[2] = 0x00;
    szTxData[3] = 0x04; // read 1024 bytes
    szTxData[4] = 0x00;
    
    IicWriteData(SLAVE_I2C_ID_DWI2C, szTxData, 5);
...
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareC32(u8 szFwData[][1024], EmemType_e eEmemType)
{
    ...
    // Erase  all
    /////////////////////////
    _DrvFwCtrlMsg21xxaEraseEmemC32();
    mdelay(1000); 
        ...
        while (nRemainSize > 0)
        {
            ...
            if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05)
            {
                ...
                for (i = 0; i < nBlockSize; i ++)
                {
                    for (j = 0; j < 3; j ++)
                    {
                        szTxData[3+(i*3)+j] = _gOneDimenFwData[index*nSizePerWrite+i];
                    }
                    nSize = nSize + 3;
                }
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
                ...
                for (i = 0; i < nBlockSize; i ++)
                {
                    for (j = 0; j < 4; j ++)
                    {
                        szTxData[3+(i*4)+j] = _gOneDimenFwData[index*nSizePerWrite+i];
                    }
                    nSize = nSize + 4;
                }
#else //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_C
                for (i = 0; i < nBlockSize; i ++)
                {
                    szTxData[3+i] = _gOneDimenFwData[index*nSizePerWrite+i];
                    nSize ++; 
                }
#endif //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A
            }
        ...
        while (nRemainSize > 0)
        {
            ...
            if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05)
            {
                ...
                for (i = 0; i < nBlockSize; i ++)
                {
                    for (j = 0; j < 3; j ++)
                    {
                        szTxData[3+(i*3)+j] = _gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE*1024)+(index*nSizePerWrite)+i];
                    }
                    nSize = nSize + 3;
                }
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
                ...
                for (i = 0; i < nBlockSize; i ++)
                {
                    for (j = 0; j < 4; j ++)
                    {
                        szTxData[3+(i*4)+j] = _gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE*1024)+(index*nSizePerWrite)+i];
                    }
                    nSize = nSize + 4;
                }
#else //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_C
                for (i = 0; i < nBlockSize; i ++)
                {
                    szTxData[3+i] = _gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE*1024)+(index*nSizePerWrite)+i];
                    nSize ++; 
                }
#endif //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A
            }
            ...
            IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], nSize);
...
void DrvFwCtrlOptimizeCurrentConsumption(void)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG22XX)
    {
        ...
        szTxData[0] = 0x10;
        szTxData[1] = 0x11;
        szTxData[2] = 0xA0; //bank:0x11, addr:h0050
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+24);  // Write 0x11 for reg 0x1150~0x115B

        szTxData[0] = 0x10;
        szTxData[1] = 0x11;
        szTxData[2] = 0xB8; //bank:0x11, addr:h005C
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+6);   // Write 0xFF for reg 0x115C~0x115E 
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX/* || g_ChipType == CHIP_TYPE_MSG58XXA*/)
    {
        ...
        szTxData[0] = 0x10;
        szTxData[1] = 0x15;
        szTxData[2] = 0x20; //bank:0x15, addr:h0010
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+8);  // Write 0xFF for reg 0x1510~0x1513

        szTxData[0] = 0x10;
        szTxData[1] = 0x15;
        szTxData[2] = 0x28; //bank:0x15, addr:h0014
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+16);   // Write 0x00 for reg 0x1514~0x151B
    
        szTxData[0] = 0x10;
        szTxData[1] = 0x21;
        szTxData[2] = 0x40; //bank:0x21, addr:h0020
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+8);  // Write 0xFF for reg 0x2120~0x2123

        szTxData[0] = 0x10;
        szTxData[1] = 0x21;
        szTxData[2] = 0x20; //bank:0x21, addr:h0010
        ...
        IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0], 3+32);  // Write 0xFF for reg 0x2110~0x211F
...
//            DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c \n", i*4+3, szRxData[i*4+3], szRxData[i*4+3]); // add for debug
    ...
    else if (g_ChipType == CHIP_TYPE_MSG26XXM)   
    {
        ...
        {
            szTxData[1] = 0x80;
            szTxData[2] = 0x10 + ((i*8)&0x00ff);

            IicWriteData(SLAVE_I2C_ID_DWI2C, &szTxData[0], 5);

            mdelay(50);

            IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[i*8], 8);
        }
        ...
        sprintf(*ppVersion, "%.16s", szRxData);
...
s32 DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)    
    {
...
u16 DrvFwCtrlChangeFirmwareMode(u16 nMode)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)  
    {
        u8 szTxData[2] = {0};
        u32 i = 0;
        s32 rc;

        g_IsDisableFinagerTouch = 1; // Skip finger touch ISR handling temporarily for device driver can send change firmware mode i2c command to firmware. 
        ...
        g_IsDisableFinagerTouch = 0; // Resume finger touch ISR handling after device driver have sent i2c command to firmware. 
    }
...
void DrvFwCtrlSelfGetFirmwareInfo(SelfFirmwareInfo_t *pInfo) // for MSG21xxA/MSG22xx
{
    ...
    g_IsDisableFinagerTouch = 1; // Skip finger touch ISR handling temporarily for device driver can send get firmware info i2c command to firmware. 
...
void DrvFwCtrlMutualGetFirmwareInfo(MutualFirmwareInfo_t *pInfo) // for MSG26xxM/MSG28xx
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG26XXM)   
    {
        ...
        if (szRxData[0] == 8 && szRxData[1] == 0 && szRxData[2] == 9 && szRxData[3] == 0)
        {
...
*/
        ...
        else if (szRxData[0] == 0xAB)
        {
            DBG(&g_I2cClient->dev, "*** Debug Mode Packet Header is 0xAB ***\n");

            pInfo->nLogModePacketHeader = szRxData[0];
            pInfo->nMy = szRxData[1];
            pInfo->nMx = szRxData[2];
//            pInfo->nSd = szRxData[1];
//            pInfo->nSs = szRxData[2];
        }
        else if (szRxData[0] == 0xA7 && szRxData[3] == PACKET_TYPE_TOOTH_PATTERN) 
        {
            DBG(&g_I2cClient->dev, "*** Debug Packet Header is 0xA7 ***\n");

            pInfo->nLogModePacketHeader = szRxData[0];
            pInfo->nType = szRxData[3];
            pInfo->nMy = szRxData[4];
            pInfo->nMx = szRxData[5];
            pInfo->nSd = szRxData[6];
            pInfo->nSs = szRxData[7];
        }
...
s32 DrvFwCtrlEnableProximity(void)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)
    {
        szTxData[2] = 0x47; 
    }
    else
...
void DrvFwCtrlOpenGloveMode(void) // used for MSG28xx only
{
    ...
    g_IsDisableFinagerTouch = 1; // Skip finger touch ISR handling temporarily for device driver can send i2c command to firmware.
...
//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);  // add for debug
    ...
    if (g_IsDisableFinagerTouch == 1)
    {
        //printk("kls Skip finger touch for handling get firmware info or change firmware mode\n");
        return;
    }
...
//    DBG(&g_I2cClient->dev, "*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug
    ...
    {	
...
#endif //CONFIG_ENABLE_HOTKNOT		
            {
               // printk("kls FIRMWARE_MODE_DEMO_MODE\n");
        ...
        {
            // printk("kls FIRMWARE_MODE_DEBUG_MODE\n");
        ...
        {
           // printk("kls WRONG FIRMWARE MODE : 0x%x\n", g_FirmwareMode);
            goto TouchHandleEnd;		
        }
    }
    else
    ...
    if (0 == _DrvFwCtrlMutualParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if (tInfo.nKeyCode != 0xFF)   //key touch pressed
        {
            ...
            if (tInfo.nKeyCode < MAX_KEY_NUM)
            {
                ...
                else
                {
                    ...
                    printk("kls REPEATED KEY\n");
                }
            }
            else
            {
                printk("kls WRONG KEY\n");
            }
        }
        ...
        printk("kls tInfo.nCount = %d, nLastCount = %d\n", tInfo.nCount, nLastCount);
        ...
        if (g_IsEnableReportRate == 1)
        {
            ...
            {
                g_ValidTouchCount = 0; // Reset count if overflow
                printk("kls g_ValidTouchCount reset to 0\n");
            } 	

            g_ValidTouchCount ++;

            printk("kls g_ValidTouchCount = %d\n", g_ValidTouchCount);
        }
#endif //CONFIG_ENABLE_COUNT_REPORT_RATE
    }
