...
extern u8 g_IsDisableFinagerTouch;
                ...
                switch (pTransCmd->nCmdId)
                {
                    ...
                    case MSGTOOL_FINGERTOUCH:
                        if(pTransCmd->nSndCmdLen == 1)
                        {
                            ...
                            g_IsDisableFinagerTouch = 0; // Resume finger touch ISR handling after MTPTool APK have sent i2c command to firmware. 
                        }
                        else if(pTransCmd->nSndCmdLen == 0)
                        {
                            DBG(&g_I2cClient->dev, "*** JNI disable touch ***\n");                                                
                            DrvPlatformLyrDisableFingerTouchReport();
                            g_IsDisableFinagerTouch = 1; // Skip finger touch ISR handling temporarily for MTPTool APK can send i2c command to firmware. 
                        }
                        break;
