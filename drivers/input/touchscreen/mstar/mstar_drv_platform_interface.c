...
/*=============================================================*/
extern int tp_has_powered;/*Add by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
...
/*=============================================================*/
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
u8 gPsSuspendReturn = 0;
#endif

#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
static bool lct_get_androidBootMode(void)
{
...
static void lct_check_tp_alive(void)
{
    ...
    while (i < 3)
    {
        ...
        {
            printk("kls Un-recognized chip type = 0x%x\n", g_ChipType);
            break;
        }
            
        if (rc > 0)
        {
            printk("kls lct tp check success\n");
            break;
        }
    ...
    if (i == 3)
    {
        printk("kls lct tp check failed, rc = %d\n", rc);
    }
    ...
    if (pEventData && pEventData->data && nEvent == FB_EVENT_BLANK)
    {
        ...
        if (*pBlank == FB_BLANK_UNBLANK)
        {
            ...
            if (g_IsUpdateFirmware != 0) // Check whether update frimware is finished
            {
            	...
            	if(gPsSuspendReturn)
		{
			printk("kls g_EnableTpProximity = %d\n", g_EnableTpProximity);

            if(lct_get_androidBootMode() == 1)
                return 0;
			...
			if(g_EnableTpProximity != 0) //Proximity is enable 
			return 0;
            // proximity is disable in suspend
		}
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
            ...
            if(tp_has_powered == 0){/*Mod by jiatianbao 20170302 ---Solve the problem that press power key proximity invalid in calling*/
                DrvPlatformLyrTouchDeviceRegulatorPowerOn(true);
